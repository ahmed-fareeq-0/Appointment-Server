...
export const register_doctor = async (req: Request, res: Response) => {
    const errors = validationResult(req);
    ...
    if (!errors.isEmpty()) {
        ...
        const result = await s_user_register_doctor(req, res);
        res.json(result);
    }

}

export const login_patient = async (req: Request, res: Response) => {
    const errors = validationResult(req);
